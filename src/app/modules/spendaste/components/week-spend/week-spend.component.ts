...
export class WeekSpendConponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    this.monthBalanceService.getWeekMoneyInMonth(this.year*100 + this.week).subscribe({
      next: (res) => {
      
        const data = res;

        const maxValue = Math.max(
          ...data.flatMap(d => [d.cashIncluded + d.cashExcluded, d.digitalIncluded + d.digitalExcluded])
        );
        ...
        this.chart = new Chart(this.chartRef.nativeElement, {
          ...
          options: {
            ...
            plugins: {
              legend: { position: 'top' },
              title: { display: true, text: 'Weekly Cash vs Digital Transactions' },
            },
          },
        });
    ...
    this.monthBalanceService.getTransactionRatio(this.year*100 + this.month).subscribe({
      next: (res) => {
